...
ws.onmessage = (event) => {
    if (typeof event.data === 'string') {
        ...
        try {
            ...
            if (response.status) {
                switch (response.status) {
                    case 'tts_starting':
                        console.log('TTS streaming starting for transcript:', response.transcript);
                        isStreamingAudio = true;
                        audioBufferQueue = []; // Clear queue for new audio stream
                        // Initialize TTS AudioContext if it doesn't exist or is closed
                        ...
                        break;
                    case 'tts_finished':
                        console.log('TTS streaming finished for transcript:', response.transcript);
                        ...
                        break;
                    case 'tts_error':
                        console.error('TTS error from server for transcript:', response.transcript, 'Error:', response.error);
                        agentResponseDiv.innerHTML += `<br><strong style="color: orange;">TTS Error:</strong> ${response.error}`;
                        isStreamingAudio = false;
                        audioBufferQueue = []; // Clear queue on error
                        break;
                }
            }

            // Handle general errors
            if (response.error && !response.status) { // Ensure it's not a TTS status error
        ...
        console.log('Received binary audio data from server, length:', event.data.byteLength);
        if (isStreamingAudio) {
            audioBufferQueue.push(event.data);
            console.log('Audio chunk added to queue. Queue length:', audioBufferQueue.length);
        } else {
            console.warn('Received audio data when not in streaming state, discarding.');
        }
    } else {
